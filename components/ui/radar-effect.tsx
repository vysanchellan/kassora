      ...
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(14, 165, 233, ${0.4 - idx * 0.04})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-blue-800/50 bg-navy-900/80 shadow-inner backdrop-blur-sm"
        style={{ background: 'rgba(4,13,42,0.8)', borderColor: 'rgba(14,165,233,0.25)' }}>
        {icon || (
      ...
      <div className="hidden rounded-md px-2 py-1 md:block">
        <div className="text-center text-xs font-medium" style={{ color: '#64748b', fontFamily: 'var(--font-mono)' }}>
          {text || "Service"}
        </div>
      </div>
    </motion.div>
